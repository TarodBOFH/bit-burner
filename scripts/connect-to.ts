...
export async function main(ns: NS) {
    ns.ramOverride(ns.getScriptRam(ns.getScriptName())-2*16);
    ...
    if (ns.singularity && ns.singularity.connect) {
        ns.singularity.connect(chosenPath[0]); // start node
        for (let i = 1; i < chosenPath.length; i++) {
            await ns.sleep(100);
            ns.singularity.connect(chosenPath[i]);
        }
    } else {
